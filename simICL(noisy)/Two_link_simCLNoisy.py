...
import numpy as np
import Two_link_dynamicsCLnoisy
import csv
import os
import datetime
import matplotlib
...
if __name__=='__main__':
    ...
    alpha= 1.0*np.ones(2,dtype=np.float32)
    beta= 0.5*np.ones(2,dtype=np.float32)
    gamma= 0.25*np.ones(5,dtype=np.float32)
    lambdaCL=1.0
    YYmindiff=0.5
    kCL=0.2
    ...
    dyn= Two_link_dynamicsCLnoisy.dynamics(alpha=alpha, beta=beta, gamma=gamma, lambdaCL=lambdaCL, YYminDiff=YYmindiff, kCL=kCL,addNoise=addNoise,tauN=tauNoise,phiN=phiNoise,phiDN=phiDNoise,phiDDN=phiDDNoise)
