...
if __name__=='__main__':
    ...
    YYmindiff=0.1
    kCL=0.1
    dyn= Two_link_dynamicsICL.dynamics(alpha=alpha, beta=beta, gamma=gamma, lambdaCL=lambdaCL, YYminDiff=YYmindiff, kCL=kCL)
    ...
    thetaTildaHist=np.zeros((5,len(t)),dtype=np.float32)
    thetaTildaNormHist=np.zeros_like(t)
    thetaClMinHist=np.zeros_like(t)
    ...
    for jj in range(0, len(t)):
        ...
        ej,_,rj,thetaTildaj = dyn.getErrorStates(t[jj])
        tauj,_,tauffj, taufbj,thetaCLj = dyn.getTauThetaHD(t[jj])
        YsumMinEigj,TCLj,_,_=dyn.getCLState()
        ...
        thetaTildaHist[:,jj]=thetaTildaj
        thetaTildaNormHist[jj]=np.linalg.norm(thetaTildaj)
        lambdaCLMinHist[jj]=YsumMinEigj
        tauHist[:,jj]=tauj
        taufbHist[:,jj]=taufbj
        tauffHist[:,jj]=tauffj
 ...
 #plot the parameter estiamtes
    ...
    thetaNax.plot(t,thetaTildaNormHist,color='orange',linewidth=2,linestyle='-')
