import numpy as np
import two_link_two_layer_nn_dynamics
import csv
import os
import datetime
import matplotlib
...
if __name__ == '__main__':
    ...
    l = 20
    L = l+1
    dyn = two_link_two_layer_nn_dynamics.Dynamics(alpha=alpha,betar=betar,betadel=betadel,gammath=gammath,gammaw=gammaw,gammav=gammav,lambdaCL=lambdaCL,YYminDiff=YYminDiff,kCL=kCL,tauN=tauNoise,phiN=phiNoise,phiDN=phiDNoise,phiDDN=phiDDNoise,l=l,deltaT=deltaT,useCL=useCL,useNN=useNN,useYth=useYth,useBias=useBias)
    ...
    eNplot,eNax = plot.subplots()
    eNax.plot(t,rNormHist,color='orange',linewidth=2,linestyle='-')
    eNax.set_xlabel("$t$ $(sec)$")
    eNax.set_ylabel("$\Vert e \Vert$")
    eNax.set_title("Filter Tracking Error Norm RMS = "+str(np.around(np.sqrt(np.mean(rNormHist**2)),decimals=2)))
    eNax.grid()
