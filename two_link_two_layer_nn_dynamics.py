...
from two_link_two_layer_nn_integral_concurrent_learning import ConcurrentLearning
...
class Dynamics():
    ...
    def getTaud(self,phi,phiD):
        taud1 = np.array([(5.0*phi[0]**3 + 5.0*phi[0]), (5.0*phi[1]**3 + 5.0*phi[1])]) #taudSpring
        taud2 = np.array([5.0*np.tanh(5.0*phiD[0])*phiD[0]**2 + 5.0*phiD[0], 5.0*np.tanh(5.0*phiD[1])*phiD[1]**2  + 5.0*phiD[1]]) #taudFriction
        taud3 = np.array([2*self.g*cos(phi[0])+self.g*np.cos(phi[0]+phi[1]), self.g*np.cos(phi[0]+phi[1])]) #taudForce
